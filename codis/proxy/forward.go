...
func (d *forwardSync) process(s *Slot, r *Request, hkey []byte) (*BackendConn, error) {
	if s.backend.bc == nil {
		config.Logger.Logf(logger.ERROR, "slot-%04d is not ready: hash key = '%s'",
			s.id, hkey)
		return nil, ErrSlotIsNotReady
	}
	if s.migrate.bc != nil && len(hkey) != 0 {
		if err := d.slotsmgrt(s, hkey, r.Database, r.Seed16()); err != nil {
			config.Logger.Logf(logger.DEBUG, "slot-%04d migrate from = %s to %s failed: hash key = '%s', database = %d, error = %s",
...
func (d *forwardSemiAsync) process(s *Slot, r *Request, hkey []byte) (_ *BackendConn, retry bool, _ error) {
	if s.backend.bc == nil {
		config.Logger.Logf(logger.DEBUG, "slot-%04d is not ready: hash key = '%s'",
			s.id, hkey)
	...
	if s.migrate.bc != nil && len(hkey) != 0 {
		...
		switch {
		case err != nil:
			config.Logger.Logf(logger.DEBUG, "slot-%04d migrate from = %s to %s failed: hash key = '%s', error = %s",
...
func (d *forwardHelper) slotsmgrt(s *Slot, hkey []byte, database int32, seed uint) error {
	...
	case resp.IsInt():
		config.Logger.Logf(logger.DEBUG, "slot-%04d migrate from %s to %s: hash key = %s, database = %d, resp = %s",
