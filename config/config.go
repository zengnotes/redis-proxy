...
import (
	...
	"github.com/zengnotes/redis-proxy/codis/utils/bytesize"
	"github.com/zengnotes/redis-proxy/codis/utils/errors"
	"github.com/zengnotes/redis-proxy/codis/utils/timesize"
...
proxy_log_path = "log/access.log"
proxy_log_maxsize = "256mb"
#"FNST", "FINE", "DEBG", "TRAC", "INFO", "WARN", "EROR", "CRIT"
proxy_log_level = "DEBG"
...
type Config struct {
	...
	ProxyLogPath                  string            `toml:"proxy_log_path" json:"proxy_log_path"`
	ProxyLogMaxsize               bytesize.Int64 `toml:"proxy_log_maxsize" json:"proxy_log_maxsize"`
	ProxyLogLevel                 string        `toml:"proxy_log_level" json:"proxy_log_level"`
...
func NewDefaultConfig() *Config {
	...
	if _, err := toml.Decode(DefaultConfig, c); err != nil {
		log.Fatal(logger.ERROR, "decode toml failed")
	}
	if err := c.Validate(); err != nil {
		log.Fatal(logger.ERROR, "validate config failed")
	}
	return c
}
...
func (c *Config)InitLogwriter() {
	//日志初始化
	name := "access.log"
	os.MkdirAll(c.ProxyLogPath, 0777)
	fileLog := logger.NewFileLogWriter(path.Join(c.ProxyLogPath, name), true)
	if c.ProxyLogMaxsize > 0 {
		fileLog = fileLog.SetRotateSize(int(c.ProxyLogMaxsize.Int64()))
	} else {
		fileLog = fileLog.SetRotateDaily(true)
	}

	if logger.DEBUG == logger.StringToLevel(c.ProxyLogLevel) {
		Logger = &logger.Logger{
			"stdout": &logger.Filter{logger.DEBUG, logger.NewConsoleLogWriter()},
		}
	} else {
		Logger = &logger.Logger{
			"fileout": &logger.Filter{logger.StringToLevel(c.ProxyLogLevel), fileLog},
		}
	}
}
