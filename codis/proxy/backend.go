...
import (
	...
	"github.com/zengnotes/redis-proxy/codis/proxy/redis"
	"github.com/zengnotes/redis-proxy/codis/utils/errors"
	"github.com/zengnotes/redis-proxy/codis/utils/math2"
	"github.com/zengnotes/redis-proxy/codis/utils/sync2/atomic2"
	"log"
...
func NewBackendConn(addr string, database int, config *config.Config) *BackendConn {
	bc := &BackendConn{
	...
	case stateDataStale:
		...
		keepAliveCallback <- func() {
			...
			var err = func() error {
				...
				case resp.IsBulkBytes():
					...
					if bc.state.CompareAndSwap(stateDataStale, stateConnected) {
						config.Logger.Logf(logger.WARNING, "backend conn [%p] to %s, db-%d state = Connected (keepalive)",
			...
			if err != nil && bc.closed.IsFalse() {
				config.Logger.Logf(logger.WARNING, "backend conn [%p] to %s, db-%d recover from DataStale failed",
					bc, bc.addr, bc.database)
			}
		}
	}
...
func (bc *BackendConn) newBackendReader(round int, config *config.Config) (*redis.Conn, chan <- *Request, error) {
...
func (bc *BackendConn) run() {
	config.Logger.Logf(logger.DEBUG, "backend conn [%p] to %s, db-%d start service",
		bc, bc.addr, bc.database)
	for round := 0; bc.closed.IsFalse(); round++ {
		config.Logger.Logf(logger.DEBUG, "backend conn [%p] to %s, db-%d round-[%d]",
			bc, bc.addr, bc.database, round)
		if err := bc.loopWriter(round); err != nil {
			bc.delayBeforeRetry()
		}
	}
	config.Logger.Logf(logger.DEBUG, "backend conn [%p] to %s, db-%d stop and exit",
		bc, bc.addr, bc.database)
}
...
func (bc *BackendConn) loopReader(tasks <-chan *Request, c *redis.Conn, round int) (err error) {
	defer func() {
		...
		config.Logger.Logf(logger.DEBUG, "backend conn [%p] to %s, db-%d reader-[%d] exit %v",
			bc, bc.addr, bc.database, round, err)
	}()
	for r := range tasks {
		resp, err := c.Decode()
		if err != nil {
		...
		if resp != nil && resp.IsError() {
			...
			case bytes.HasPrefix(resp.Value, errMasterDown):
				if bc.state.CompareAndSwap(stateConnected, stateDataStale) {
					config.Logger.Logf(logger.WARNING, "backend conn [%p] to %s, db-%d state = DataStale, caused by 'MASTERDOWN'",
						bc, bc.addr, bc.database)
				}
			}
		}
...
func (bc *BackendConn) loopWriter(round int) (err error) {
	defer func() {
		...
		config.Logger.Logf(logger.WARNING, "backend conn [%p] to %s, db-%d writer-[%d] exit",
			bc, bc.addr, bc.database, round)
	}()
...
func newSharedBackendConn(addr string, pool *sharedBackendConnPool) *sharedBackendConn {
	...
	if err != nil {
		config.Logger.Logf(logger.WARNING, "split host-port failed, address = %s  %v", addr, err)
	}
	s := &sharedBackendConn{
		addr: addr,
...
func newSharedBackendConnPool(config *config.Config, parallel int) *sharedBackendConnPool {
