...
func NewSession(sock net.Conn, conf *config.Config) *Session {
	c := redis.NewConn(sock,
		conf.SessionRecvBufsize.AsInt(),
		conf.SessionSendBufsize.AsInt(),
	)
	c.ReaderTimeout = conf.SessionRecvTimeout.Duration()
	c.WriterTimeout = conf.SessionSendTimeout.Duration()
	c.SetKeepAlivePeriod(conf.SessionKeepAlivePeriod.Duration())
	...
	config.Logger.Logf(logger.INFO, "session [%p] create: %s", s, s)
	return s
}
...
func (s *Session) CloseReaderWithError(err error) error {
	s.exit.Do(func() {
		if err != nil {
			config.Logger.Logf(logger.INFO, "session [%p] closed: %s, error: %s", s, s, err)
...
func (s *Session) CloseWithError(err error) error {
	s.exit.Do(func() {
		if err != nil {
			...
			config.Logger.Logf(logger.INFO, "session [%p] closed: %s, quit", s, s)
		}
	})
	s.broken.Set(true)
	return s.Conn.Close()
...
func (s *Session) Start(d *Router) {
	s.start.Do(func() {
		...
		config.Logger.Logf(logger.DEBUG, "router isOnline: %v ", d.isOnline())
		if !d.isOnline() {
			go func() {
