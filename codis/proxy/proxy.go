...
import (
	...
	"github.com/zengnotes/redis-proxy/codis/utils/unsafe2"
	"github.com/zengnotes/redis-proxy/codis/utils"
...
func New(conf *config.Config) (*Proxy, error) {
	...
	//路由规则
	s.router = NewRouter(conf)

	//Set heap placeholder to reduce GC frequency. 设置堆占位符以降低GC频率
	//GC  256mb
	s.ignore = make([]byte, conf.ProxyHeapPlaceholder.Int64())
	/*
	...
	*/
	...
	unsafe2.SetMaxOffheapBytes(conf.ProxyMaxOffheapBytes.Int64())
...
func (s *Proxy) setup(config *config.Config) error {
	...
	if l, err := net.Listen(proto, config.ProxyAddr); err != nil {
		...
		/*x*/
		_, err = utils.ReplaceUnspecifiedIP(proto, l.Addr().String(), config.HostProxy)
		if err != nil {
			return err
		}
...
func (s *Proxy) serveProxy() {
	...
	config.Logger.Logf(logger.DEBUG, "[%p] proxy start service on %s", s, s.lproxy.Addr())
	//启用监听
	eh := make(chan error, 1)
	...
	select {
	case <-s.exit.C:
		config.Logger.Logf(logger.DEBUG, "[%p] proxy shutdown", s)
	//log.Warnf("[%p] proxy shutdown", s)
	case err := <-eh:
		config.Logger.Logf(logger.ERROR, "[%p] proxy exit on error %v", s, err)
	...
	for {
		...
		if err != nil {
			if e, ok := err.(net.Error); ok && e.Temporary() {
				config.Logger.Logf(logger.ERROR, "[%p] proxy accept new connection failed", s)
				delay.Sleep()
				continue
			}
		}
